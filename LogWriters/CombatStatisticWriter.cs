			...
			using (StreamWriter s = new StreamWriter(stream))
			{
				for (int i = 0; i < log.Count; i++)
				{
					...
					else if (pak is StoC_0x6C_KeepComponentOverview)
					{
    					...
    					oidInfo[keep.Uid] = new ObjectInfo(eObjectType.keep, string.Format("keepId:0x{0:X4} componentId:{1}", keep.KeepId, keep.ComponentId), 0);
					}
					...
					else if (pak is StoC_0x16_VariousUpdate)
					{
						...
						if (stat.SubCode == 3)
						{
							...
							s.WriteLine("{0, -16} 0x16:3 nameKey:{1} plrName:{2} {3} {4}", pak.Time.ToString(), nameKey, plrName, plrLevel, plrClass);
						}
						...
						else if (stat.SubCode == 5)
						{
							...
							statKey = key;
							s.WriteLine("{0, -16} 0x16:5 S:{1} {2} {3} {4} {5}", pak.Time.ToString(), statKey, playerInfo.name, playerInfo.level, playerInfo.weaponDamage, playerInfo.weaponSkill);
						}
						// Fill styles
						if (stat.SubCode == 1)
						{
...
/* 							foreach (DictionaryEntry entry in styleIcons)
					...
					else if (pak is StoC_0xBC_CombatAnimation && (playerInfo != null))
					{
						...
						string styleName = (combat.StyleId == 0 /*  || (combat.Result & 0x7F) != 0x0B)*/) ? "" : (styleIcons[combat.StyleId] == null ? "not found " + combat.StyleId.ToString() : (styleIcons[combat.StyleId]).ToString());
...
//										CombatWaitMessage.Add(new WaitMessage(0x11, targetObj.GetFirstFullName + " The Midgardian Assassin attacks you and you block the blow!"));
								...
								case 0xA:
									if (targetObj != null)
										CombatWaitMessage.Add(new WaitMessage(0x11, "You attack " + targetObj.GetFullName + " with your % and hit for % damage!"));
									break;
								case 0xB:
						...
						if (combat.AttackerOid == playerOid)
						{
							s.WriteLine("{0, -16} 0xBC attackerOid:0x{1:X4}(You) defenderOid:0x{2:X4} style:0x{4:X4} result:0x{3:X2}{5}{6}",
								pak.Time.ToString(), combat.AttackerOid, combat.DefenderOid, combat.Result, combat.StyleId, styleName == "" ? "" : " styleName:" + styleName, targetName);
