...
{
    [LogPacket(0x83, 1124, ePacketDirection.ServerToClient, "Quest update v1115")]
    public class StoC_0x83_QuestUpdate_1124 : StoC_0x83_QuestUpdate_1115
    {
        protected override void InitQuestUpdate()
        {
            subData = new QuestUpdate_1124();
            subData.Init(this);
        }

        public class QuestUpdate_1124 : QuestUpdate_1109
        {
        ...
        {
            subData = new NewQuestUpdate_1124();
            subData.Init(this);
        }

        public class NewQuestUpdate_1124 : NewQuestUpdate_1115
        {
            ...
            public override void MakeString(TextWriter text, bool flagsDescription)
            {
                ...
                for (int i = 0; i < goalsCount; i++)
                {
                    ...
                    if (item.slot > 0)
                    {
                        text.Write("\n\tslot:{0,-2} level:{1,-2} value1:0x{2:X2} value2:0x{3:X2} hand:0x{4:X2} damageType:0x{5:X2} objectType:0x{6:X2} weight:{7,-4} con:{8,-3} dur:{9,-3} qual:{10,-3} bonus:{11,-2} model:0x{12:X4} color:0x{13:X4} effect:0x{14:X2} flag:0x{15:X2} extension:{16} itemID:{18} \"{17}\"",
                            item.slot, item.level, item.value1, item.value2, item.hand, item.damageType, item.objectType, item.weight, item.condition, item.durability, item.quality, item.bonus, item.model, item.color, item.effect, item.flag, item.extension, item.name, item.itemID);
        ...
        public StoC_0x83_QuestUpdate_1124(int capacity) : base(capacity)
        {
        }
    }
}
