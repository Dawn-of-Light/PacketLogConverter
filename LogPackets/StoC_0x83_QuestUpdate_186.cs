		...
		public class NewQuestUpdate_186 : NewQuestUpdate
		{
			...
			public override void Init(StoC_0x83_QuestUpdate pak)
			{
				...
				for (int i = 0; i < goalsCount; i++)
				{
					...
					questGoalInfo.unk1 = pak.ReadInt();
					questGoalInfo.unk2 = pak.ReadInt();
					questGoalInfo.unk3 = pak.ReadInt();
			...
			public override void MakeString(StringBuilder str, bool flagsDescription)
			{
				...
				for (int i = 0; i < goalsCount; i++)
				{
					...
					str.AppendFormat("\n\tinfo: unk1:0x{0:X8} unk2:0x{1:X8} unk3:0x{2:X8}",
						questGoalInfo.unk1, questGoalInfo.unk2, questGoalInfo.unk3);
					str.AppendFormat(" zoneId:{0} @X:{1} @Y:{2}",
