		...
		public class NewQuestUpdate_1115 : NewQuestUpdate_1109
		{
			...
			public override void MakeString(TextWriter text, bool flagsDescription)
			{
				...
				for (int i = 0; i < goalsCount; i++)
				{
					...
					if (item.slot > 0)
					{
						text.Write("\n\tslot:{0,-2} level:{1,-2} value1:0x{2:X2} value2:0x{3:X2} hand:0x{4:X2} damageType:0x{5:X2} objectType:0x{6:X2} weight:{7,-4} con:{8,-3} dur:{9,-3} qual:{10,-3} bonus:{11,-2} bonuslvl:{19,-2} model:0x{12:X4} color:0x{13:X4} effect:0x{14:X2} flag:0x{15:X2} extension:{16} itemID:{18:X2} \"{17}\"",
							item.slot, item.level, item.value1, item.value2, item.hand, item.damageType, item.objectType, item.weight, item.condition, item.durability, item.quality, item.bonus, item.model, item.color, item.effect, item.flag, item.extension, item.name, item.itemID, item.bonus_level);
